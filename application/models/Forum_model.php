...
class Forum_model extends CI_Model {
	...
	public function delete_forum($id)
	{
		...
		if ($this->db->delete('forums')) {
			...
			$gettopic = $this->db->get('topics');
			if ($gettopic->num_rows() == 0) {
				# code...
				return false;
			}else if($gettopic->num_rows() == 1){				
				foreach ($gettopic->result() as $a) {
					# code...
					$this->db->where('topic_id', $a->id);
				...
				$this->db->select('id');
				$this->db->where('forum_id', $id);
				$topicid = $this->db->get('topics')->result();
				foreach ($topicid as $a) {
					# code...
					$this->db->where('id', $a);
					if ($this->db->delete('posts')) {
						return false;
